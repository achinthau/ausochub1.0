<div class="flex space-x-4 justify-around" wire:key='{{$id}}'>
    @php
        $url = '';
        $hasFile = Storage::disk('asterisk-media-server')->has($uniqueid . '.wav');
        if ($hasFile) {
            // $url = Storage::disk('asterisk-media-server')->url($uniqueid . '.wav');
    ...
    {{-- @dump($url) --}}
    @if ($hasFile)
        <a href="#" class="my-auto" role="button" if wire:click.prevent="download('{{ $uniqueid }}')">
