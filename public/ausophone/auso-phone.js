var AusoPhoneBundle = (() => {
  ...
  __export(index_exports, {
    ...
    AusoPhone: () => AusoPhone,
    AusoPhoneElement: () => AusoPhoneElement,
    CallState: () => CallState,
...
`;
  ...
  var ALL_EVENTS = Object.values(PhoneEvents).concat([TRANSFER_UMBRELLA]);

  // src/SIPClient.js
  var log = createLogger("SIPClient");
  var SIPClient = class {
    /**
     * @param {object} deps
  ...
  // src/RegistrationManager.js
  var log2 = createLogger("RegistrationManager");
  var RegistrationManager = class {
      ...
      await this.registerer.register({
        requestDelegate: {
          onReject: (response) => {
            ...
            log2.error(`REGISTER rejected ${code} ${reason}`);
    ...
    async unregister() {
      ...
      try {
        ...
        log2.warn("unregister failed", err);
      }
    }
    async dispose() {
      if (!this.registerer) return;
  ...
  var Call = class {
    ...
    get durationSeconds() {
      ...
      return Math.floor((end - this.answeredAt) / 1e3);
    }
    get peerConnection() {
  ...
  function parseIdentity(nameAddrHeader) {
  ...
  // src/CallManager.js
  var log3 = createLogger("CallManager");
  var CallManager = class {
    ...
    handleInvite(invitation) {
      ...
      invitation.progress().catch((err) => log3.warn("progress() failed", err));
      if (this.autoAnswer) {
        log3.info(`auto-answering ${call.id} in ${this.autoAnswerDelayMs}ms`);
        setTimeout(() => {
          if (this.calls.get(call.id)?.state === CallState.RINGING) {
            this.answer(call.id, { auto: true }).catch((err) => log3.error("auto-answer failed", err));
          }
    ...
    async hangup(callId) {
      ...
      try {
        ...
        log3.warn(`hangup on ${call.id} failed`, err);
      }
    ...
    async hold(callId, held = true) {
      const call = this._require(callId);
      if (call.session.state !== SessionState2.Established) {
      ...
      await call.session.invite({
        ...
        requestDelegate: {
          onReject: () => {
            log3.warn(`re-INVITE for hold=${held} rejected`);
            call.held = !held;
    ...
    async sendDTMF(callId, tones, { mode = "auto", duration = 100 } = {}) {
      ...
      if (mode !== "info" && typeof sdh?.sendDtmf === "function") {
        ...
        if (!sent) log3.warn("RTP DTMF unavailable, falling back to SIP INFO");
      }
      if (!sent) {
...
Duration=${duration}`
    ...
    _wireSession(call) {
      ...
      call.session.delegate = {
        ...
        onRefer: (referral) => {
          log3.info(`incoming REFER on ${call.id}`);
          referral.accept().then(() => referral.makeInviter().invite()).catch((err) => log3.error("inbound REFER failed", err));
        }
      };
    }
    ...
    _end(call, reason) {
      ...
      setTimeout(() => this.calls.delete(call.id), 5e3);
      this._ensureDurationTimer();
    }
  ...
  // src/TransferManager.js
  var log4 = createLogger("TransferManager");
  var TransferManager = class {
      ...
      return new Promise((resolve, reject) => {
        call.session.refer(uri, {
          ...
          onNotify: (notification) => {
            ...
            log4.debug(`blind transfer NOTIFY: ${status ?? "unparsed"}`);
            ...
            if (status && status >= 200 && status < 300) {
              ...
              this._emit(PhoneEvents.TRANSFER_FAILED, {
                ...
                target,
                code: status,
                reason: "Transfer target rejected",
                call: call.toJSON()
              });
            }
          },
          requestDelegate: {
            onAccept: () => {
              log4.info(`blind transfer to ${target} accepted (202)`);
              ...
              this._blindFallback = setTimeout(() => {
                if (this.calls.calls.get(call.id)?.isActive) {
                  log4.warn("PBX did not hang up our leg after the transfer; doing it locally");
            ...
            onReject: (response) => {
              ...
              log4.error(`blind transfer rejected ${code} ${reason}`);
              this._emit(PhoneEvents.TRANSFER_FAILED, {
                type: "blind",
                target,
                code,
    ...
    async completeAttended() {
      ...
      if (!consult || consult.session.state !== SessionState2.Established) {
        await this._abort("Consultation call is not established");
        throw new Error("Consultation call is not established");
      }
      return new Promise((resolve, reject) => {
        primary.session.refer(consult.session, {
          requestDelegate: {
            onAccept: () => {
              log4.info(`attended transfer to ${target} accepted`);
              this._emit(PhoneEvents.TRANSFER_COMPLETED, {
                type: "attended",
                target,
                call: primary.toJSON()
    ...
    async cancelAttended() {
      ...
      if (primary && primary.isActive && primary.held) {
        await this.calls.unhold(primary.id).catch((err) => log4.warn("unhold after cancel failed", err));
      }
      this._emit(PhoneEvents.TRANSFER_FAILED, {
        type: "attended",
        target,
    ...
    notifyCallEnded(callId) {
      ...
      if (callId === this.pending.consultCallId && this.pending.stage !== "completed") {
        log4.warn("consultation leg ended \u2014 reverting customer off hold");
        this.cancelAttended().catch(() => {
        });
      } else if (callId === this.pending.primaryCallId) {
        log4.warn("customer leg ended during transfer");
  ...
  // src/MediaManager.js
  var log5 = createLogger("MediaManager");
  var MediaManager = class {
    ...
    assertSecureContext() {
      ...
      throw new Error(
        window.isSecureContext ? `This browser does not support getUserMedia (origin ${origin}).` : `Microphone access needs a secure context, and ${origin} is not one. Open the console on http://localhost (or serve it over https). Browsers only treat localhost, 127.0.0.1 and [::1] as secure origins \u2014 a LAN address or 0.0.0.0 will not work.`
      );
    }
    getConstraints() {
      const audio = {
    ...
    async setOutputDevice(deviceId) {
      ...
      if (this.remoteAudio && typeof this.remoteAudio.setSinkId === "function" && deviceId) {
        try {
          ...
          log5.warn("setSinkId failed (unsupported browser?)", err);
        }
      }
    }
    ...
    bindRemoteStream(peerConnection) {
      ...
      if (!stream.getAudioTracks().length) {
        log5.warn("no remote audio track yet");
        return;
      }
      this.remoteAudio.srcObject = stream;
      const play = this.remoteAudio.play();
      if (play?.catch) {
        play.catch((err) => log5.warn("remote audio autoplay blocked", err));
      }
    ...
    startRingback() {
      this._startTone([400, 450], { onMs: 1500, offMs: 3e3, gain: 0.25 });
    }
    _startTone(freqs, { onMs, offMs, gain = 0.35 }) {
      ...
      const cycle = () => {
        ...
        master.gain.setValueAtTime(0, now + onMs / 1e3);
      };
      cycle();
...
`;
  ...
  var log6 = createLogger("RecordingManager");
  var RecordingManager = class {
    ...
    start(call) {
      ...
      const recorder = new MediaRecorder(destination.stream, mimeType ? { mimeType } : void 0);
      const chunks = [];
      recorder.ondataavailable = (e) => {
        if (e.data && e.data.size > 0) chunks.push(e.data);
      };
      recorder.onerror = (e) => {
        log6.error("recorder error", e.error);
        this.events.emit(PhoneEvents.ERROR, { scope: "recording", message: String(e.error) });
      };
      recorder.start(1e3);
      call.recording = true;
      const info = { call_id: call.id, started_at: (/* @__PURE__ */ new Date()).toISOString(), mime_type: mimeType };
    ...
    async stop(callId, opts = {}) {
      ...
      if (shouldUpload && blob.size > 0) {
        upload = await this.upload(blob, finished, call).catch((err) => {
          log6.error("recording upload failed", err);
  ...
  // src/EventManager.js
  var log7 = createLogger("EventManager");
  var EventManager = class {
     ...
     */
    emit(event, payload = {}) {
      const enriched = { event, at: (/* @__PURE__ */ new Date()).toISOString(), ...payload };
      log7.debug(event, enriched);
      ...
      for (const handler of this.handlers.get(event) ?? []) {
        try {
          ...
          log7.error(`handler for "${event}" threw`, err);
        }
      }
      for (const handler of this.handlers.get("*") ?? []) {
        try {
          ...
          log7.error("wildcard handler threw", err);
        }
      }
      if (this.domTarget) {
        try {
          ...
          log7.error("dispatchEvent failed", err);
        }
      }
      return enriched;
    }
...
`;
  ...
  // ui/icons.js
  var svg = (paths, opts = {}) => `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="${opts.w ?? 1.8}" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${paths}</svg>`;
  var icons = {
  ...
  var AusoPhoneElement = class extends HTMLElement {
    ...
    _subscribe() {
      ...
      const watched = [
        PhoneEvents.CONNECTING,
        PhoneEvents.CONNECTED,
        PhoneEvents.DISCONNECTED,
        PhoneEvents.REGISTERED,
        PhoneEvents.UNREGISTERED,
        PhoneEvents.REGISTRATION_FAILED,
        PhoneEvents.INCOMING,
        PhoneEvents.DIALING,
        PhoneEvents.RINGING,
        PhoneEvents.ANSWERED,
        PhoneEvents.HANGUP,
        PhoneEvents.HOLD,
        PhoneEvents.UNHOLD,
        PhoneEvents.MUTE,
        PhoneEvents.UNMUTE,
        PhoneEvents.CALL_UPDATED,
        PhoneEvents.TRANSFER_STARTED,
        PhoneEvents.TRANSFER_COMPLETED,
        PhoneEvents.TRANSFER_FAILED,
        PhoneEvents.RECORDING_STARTED,
    ...
    render() {
      ...
      const active = s.calls.find((c) => c.state === CallState.ANSWERED || c.state === CallState.HELD) ?? s.calls.find((c) => c.state === CallState.DIALING || c.state === CallState.RINGING);
    ...
    _renderActive(call, s) {
      ...
      <div class="label">${esc(label)}${call.consultation ? " \xB7 consultation" : ""}</div>
      <div class="cli">${esc(call.cli)}</div>
      <div class="who">${esc(call.remote_display_name || "")}</div>
    ...
    _renderConsultControls(s) {
      const ready = s.transfer.consultCallId && s.calls.find((c) => c.call_id === s.transfer.consultCallId)?.state === CallState.ANSWERED;
    ...
    _renderCustomer(call) {
      ...
      <div class="meta">${bits.join(" \xB7 ")}</div>
    </div>`;
    }
    /** DialPad view. */
    _renderDialPad(s) {
    ...
    _renderTransferDialog(s) {
      ...
      <div class="hint">${this.transferType === "blind" ? "Sends a REFER immediately and drops you from the call." : "Holds the customer and dials the destination so you can announce the call first."}</div>
      ...
      try {
        ...
        switch (action) {
          ...
          case "dial":
            ...
            return;
          case "answer":
            return void await this.phone.answer(callId);
          case "reject":
            return void await this.phone.reject(callId);
          case "hangup":
            return void await this.phone.hangup(callId);
          case "toggle-mute":
            return void await this.phone.toggleMute(callId);
          case "toggle-hold":
            return void await this.phone.toggleHold(callId);
          case "open-transfer":
            this.view = "transfer";
            return this.render();
          case "open-dtmf":
          ...
          case "complete-transfer":
            return void await this.phone.completeTransfer();
          case "cancel-transfer":
            return void await this.phone.cancelTransfer();
          case "swap-legs":
            return void await this.phone.swapTransferLegs();
          case "toggle-record": {
  ...
  function defineAusoPhoneElement(tag = "auso-phone") {
  ...
  var index_default = phone;
  return __toCommonJS(index_exports);
})();
//# sourceMappingURL=auso-phone.js.map
