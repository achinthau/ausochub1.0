<x-modal.card title="Add Service Ticket" blur align="center" wire:model="creatingCxTicket">
        ...
        <div class="grid grid-cols-1  gap-4">
            ...
            <div class="border p-4 rounded-lg shadow-md mt-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <x-native-select label="Category" placeholder="Select Category" :options="$categories->map(
                            fn($category) => ['id' => $category->name, 'title' => $category->name],
                        )"
                            option-label="title" option-value="id" wire:model="category" />

                        <div class="py-2"><x-input label="Product" placeholder="Enter the product"
                                wire:model="product" /> </div>
                        <div class="py-2"><x-input label="Model" placeholder="Enter the product model"
                                wire:model="model" /></div>
                        <x-input label="Work Order No" placeholder="Enter the Work Order No"
                            wire:model="work_order_no" />
                    </div>
                    <div>
