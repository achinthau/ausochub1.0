    ...
    <ul class="flex flex-col divide-y w-full">
        ...
        @foreach ($users as $user)
            ...
            <li
                ...
                class="flex flex-row @if (!in_array(optional($user->user)->id, $loggedUserIds)) order-last grayscale @else @if ($user->user && $user->user->on_break) order-2 @else order-first @endif @endif ">
                <div class="select-none cursor-pointer hover:bg-gray-50 flex flex-1 items-center p-4">
                    <div class="flex flex-col w-10 h-10 justify-center items-center mr-4">
                        <a href="#" class="block relative">
                            ...
                            <svg class="mx-auto object-cover rounded-full h-10 w-10" xmlns="http://www.w3.org/2000/svg"
                                xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" viewBox="0 0 256 256"
                                xml:space="preserve"  onclick="myFunction({{ $user->user->id ?? 'null' }})" >  
                            ...
                            <g style="stroke: none; stroke-width: 0; stroke-dasharray: none; stroke-linecap: butt; stroke-linejoin: miter; stroke-miterlimit: 10; fill: none; fill-rule: nonzero; opacity: 1;"
                                ...
                                @php
                                    ...
                                    // $callDirection = $user->user
                                    //     ? \Illuminate\Support\Facades\Redis::get("user:{$user->user->id}:bound_type")
                                    //     : null;
                                    
                                    //     // echo "<script>alert('Too many outbound agents active!'{$callDirection});</script>";
                                        

                                    // if($callDirection == 'dialer')
                                    // {
                                    //     // $color = rgb(240,88,47);
                                    //     $color = '#F0582F';
                                    // }
